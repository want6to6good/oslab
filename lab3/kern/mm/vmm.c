...
//用于查找给定地址是否属于某个虚拟内存区域（VMA）。
//遍历进程的 VMA 列表，查找包含给定地址的 VMA。
struct vma_struct *
...
//用于将一个 VMA 插入到进程的 VMA 列表中。
//确保 VMA 按照起始地址排序，检查是否与其他 VMA 重叠。
void
...
//用于检查虚拟内存管理的正确性。
//创建一个内存管理结构，添加一系列虚拟内存区域，检查排序和查找操作的正确性。
static void
check_vmm(void) {
...
//用于检查虚拟内存区域的相关操作的正确性。
//创建多个 VMA，添加到内存管理结构中，并检查排序和查找操作的正确性。
static void
...
//用于检查页面错误处理的正确性。
//创建一个内存管理结构，添加一个虚拟内存区域，模拟页面错误并处理。
static void
check_pgfault(void) {
 ...
 */
//处理页面错误中断，通常在用户程序访问尚未映射的虚拟地址时触发。
//根据错误代码（包括访问权限和用户/内核模式）和地址，检查是否可以解决页面错误。
//如果需要从磁盘加载页面内容，则调用 swap_in 等相关函数来实现页面交换。
int
    ...
    if (*ptep == 0) {
        ...
        */
        if (swap_init_ok) {
             ...
             *    into the memory which page managed.
             * 第一步，根据mm与la线性地址，将正确的磁盘页中的内容加载到该页管理的内存当中
             * 即令Page指针指向成功交换后的物理页
            */
