...
alloc_proc(void) {
     ...
     */
        ...
        /* 每个线程都有一个内核栈，并且位于内核地址空间的不同位置。
         ...
         其次，内核栈位于内核地址空间，并且是不共享的（每个线程都拥有自己的内核栈）， 因此不受到 mm 的管理，当进程退出的时候，
        ...
        proc->tf = NULL;                        // 保存了进程的中断帧。进程的执行状态被保存在了中断帧中。系统调用可能会改变用户寄存器的值，我们可以通过调整中断帧来使得系统调用返回特定的值。
...
/*为进程分配一个唯一的pid，是根据当前已分配的进程号，找到下一个未被使用的进程号。
通过循环遍历进程列表，如果发现当前 last_pid 已经被使用，就将其加一并检查是否超过了下一个安全的进程号。*/
