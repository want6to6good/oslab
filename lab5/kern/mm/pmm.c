...
//检查PTE有效性
    if (*ptep & PTE_V) {  //(1) check if this page table entry is
        struct Page *page =
            pte2page(*ptep);  //(2) find corresponding page to pte找到对应的页面
        page_ref_dec(page);   //(3) decrease page reference减少页面引用计数
        if (page_ref(page) ==
            0) {  //(4) and free this page when page reference reachs 0释放页面
            free_page(page);
        }
        *ptep = 0;                  //(5) clear second page table entry清除第二级页表项
...
void unmap_range(pde_t *pgdir, uintptr_t start, uintptr_t end) {
    ...
    do {
        pte_t *ptep = get_pte(pgdir, start, 0);//获取指针
        if (ptep == NULL) {
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }
        if (*ptep != 0) {
            page_remove_pte(pgdir, start, ptep);//取消
        }
        start += PGSIZE;
    } while (start != 0 && start < end);//循环取消映射
}
//释放内存
void exit_range(pde_t *pgdir, uintptr_t start, uintptr_t end) {
    ...
    uintptr_t d1start, d0start;//表示上一级页目录和当前页目录的起始地址
    ...
    do {
        ...
        /*首先，检查一级页目录（pde1 = pgdir[PDX1(d1start)]），如果存在有效的一级页目录项，则进入二级页目录。
在二级页目录中，尝试释放所有的页表（level 0），并更新一级页目录的对应项。
如果所有页表都已经无效，则释放对应的二级页目录项，并将一级页目录中的对应项清零。*/
 ...
 */
 //在两个进程间复制特定线性地址范围内的页面内容，并将这些页面映射到目标进程的页表
 //使用互斥锁（用于避免多个进程同时访问内存）。进下一层的调用dup_mmap函数。
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
    ...
    do {
        ...
        if (*ptep & PTE_V) {
             ...
             *使用 get_pte 函数来获取源进程（Process A）地址 start 对应的页表项指针 ptep。
            如果 ptep 为 NULL，表示当前地址没有对应的页表项，可能因为它是不连续的空间，此时将起始地址向下舍入到前一个大页的边界，并继续处理下一个大页。
            如果 ptep 不为 NULL，表示源进程存在对应的有效页表项，接着尝试在目标进程（Process B）中获取对应的页表项 nptep。
            如果目标进程的页表项 nptep 为空，就分配一个页表给它。如果内存分配失败，则返回错误 -E_NO_MEM。
            通过 pte2page(*ptep) 获取源进程的页面结构，然后使用 alloc_page() 分配一个新页面给目标进程。
            使用 memcpy 将源页面的内容复制到目标页面。
            最后，使用 page_insert 将目标页面映射到目标进程的页表中。
